    ...
    <section id="faq" className="py-32 px-6 relative overflow-hidden bg-[#0d1117]">
      ...
      <div className="max-w-4xl mx-auto">
        ...
        >
          ...
          <p className="text-white/50 text-xl font-light max-w-xl mx-auto">
            ...
            return (
              <motion.div
                ...
                className={`border rounded-2xl overflow-hidden transition-all duration-300 ${
                  isOpen ? 'border-brand-accent/30 bg-white/[0.04]' : 'border-white/[0.06] bg-white/[0.02] hover:border-white/10'
                }`}
              >
                <button
                ...
                >
                  <span className={`text-base font-medium leading-snug transition-colors ${isOpen ? 'text-white' : 'text-white/70'}`}>
                    {faq.question}
                  </span>
                  <div className={`w-8 h-8 rounded-full border flex items-center justify-center shrink-0 transition-all ${
                    isOpen ? 'bg-brand-accent border-brand-accent text-white' : 'border-white/10 text-white/40'
                  }`}>
                  ...
                  {isOpen && (
                    ...
                    >
                      <div className="px-6 pb-6 pt-0">
                        ...
                        <p className="text-white/50 font-light leading-relaxed">{faq.answer}</p>
                      </div>
                    </motion.div>
                  )}
                </AnimatePresence>
        ...
        >
          <p className="text-white/40 text-sm">Still have questions?</p>
          <motion.button
            whileHover={{ scale: 1.02 }}
            whileTap={{ scale: 0.98 }}
            onClick={() => document.getElementById('contact')?.scrollIntoView({ behavior: 'smooth' })}
            className="px-10 py-4 bg-brand-accent text-white font-bold uppercase tracking-widest text-[11px] hover:bg-indigo-400 transition-all rounded-2xl"
          >
