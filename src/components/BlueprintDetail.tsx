...
import { ArrowLeft, Download, BookOpen, Calendar, Tag, ExternalLink, FileText, ChevronDown } from 'lucide-react';
...
function DownloadMenu({ onHTML, onPDF, dark = false }: { onHTML: () => void; onPDF: () => void; dark?: boolean }) {
  ...
  const btn = dark
    ? 'flex items-center gap-2 px-4 py-2 bg-brand-accent text-white text-[11px] font-bold uppercase tracking-widest rounded-xl hover:bg-brand-accent/80 transition-colors'
    : 'flex items-center gap-2 px-4 py-2 bg-brand-accent text-white text-[11px] font-bold uppercase tracking-widest rounded-xl hover:bg-brand-accent/80 transition-colors';
    ...
    <div className="relative" ref={ref}>
      ...
      {open && (
        <div className="absolute right-0 bottom-full mb-2 w-40 bg-white border border-slate-200 rounded-xl shadow-xl overflow-hidden z-50">
          <button
            onClick={() => { onHTML(); setOpen(false); }}
            className="flex items-center gap-2.5 w-full px-4 py-3 text-[12px] font-semibold text-slate-700 hover:bg-slate-50 transition-colors"
          >
              ...
              <div className="mb-10 space-y-4">
                ...
                <div className="flex flex-wrap items-center gap-3 pt-1">
                  ...
                  <div className="ml-auto flex items-center gap-2">
                    ...
                    <DownloadMenu onHTML={handleDownload} onPDF={handleDownloadPDF} dark />
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-3xl shadow-[0_0_80px_rgba(0,0,0,0.4)] overflow-hidden">
                ...
                <div className="border-t border-slate-100 px-8 py-6 md:px-14 bg-slate-50 flex flex-wrap items-center justify-between gap-4">
                  ...
                  <div className="flex gap-2">
                    ...
                    <DownloadMenu onHTML={handleDownload} onPDF={handleDownloadPDF} />
                  </div>
                </div>
              </div>
